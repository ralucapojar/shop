...
?> 	
	...
	<h3 class="price">Quantity: <?= $_SESSION['cart'][$elem_product["id"]] ?></h3><br>
	<form method="get" action="cart.php">
	<input style="display:inline;" type="number" min="1"  placeholder="<?= $_SESSION['cart'][$elem_product["id"]] ?>" name="quantity" required="required" />
	<input style="display:inline;" type="hidden" name="id" value="<?= $elem_product["id"] ?>"/>
	<input style="display:inline;" type="submit" value="<?= translate('change') ?>" />
	</form>
	<h3 class="price">Total Price: <?= $elem_product["price"] * $_SESSION['cart'][$elem_product["id"]] ?>$</h3><br>
	<img class="image" src="<?= $elem_product["img"] ?>"/><br>
	<p>Description: <br> <?= $elem_product["description"] ?></p><br>
	<?php $rmElem = 'cart.php?action=remove&id='.$elem_product['id'] ?>
	<a href="<?= $rmElem ?>"><?= translate('remove') ?></a>
	</div>
	<?php $total = $total + $elem_product["price"] * $_SESSION['cart'][$elem_product["id"]]; ?>
	<?php endif; ?>
	<?php endforeach; ?>
	<h1 class="error">Final: <?= $total ?>$ </h2>
	</div>
	<table>
